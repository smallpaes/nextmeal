  ...
  context('user actions', () => {
    ...
    describe('if user logs in', () => {
      ...
      it('should be able to post a new order', (done) => {
        request(app)
          .post('/api/order/new')
          .send('meal_id=1&require_date=2019-11-01&quantity=1')
          .expect(200)
          .end(async (err, res) => {
      ...
      it('should be able to update an order info', (done) => {
        request(app)
          .put('/api/order/1')
          .send('require_date=2019-11-02&quantity=2')
          .expect(200)
          .end(async (err, res) => {
            const order = await db.Order.findByPk(1)
            expect(order.amount).to.be.equal(2)
            expect(res.body.status).to.be.equal('success')
            expect(res.body.message).to.be.equal('Successfully edit Order information.')
            return done()
          })
      })
